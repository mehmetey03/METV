...
def get_all_channels():
    ...
    # Tüm kanal listesini bul (HTML'deki li elementleri)
    kanal_liste = soup.find_all("li", attrs={"data-url": True, "data-adi": True})
    
    print(f"Toplam {len(kanal_liste)} kanal bulundu")
    
    for kanal in kanal_liste:
        # Kanal adı ve URL parametresini al
        kanal_adi = kanal.get("data-adi", "").strip()
        kanal_param = kanal.get("data-url", "").strip()
        
        # Logo URL'sini bulmaya çalış
        logo = ""
        ...
        if img_tag and img_tag.get("src"):
            ...
            if logo.startswith("//"):
                ...
                logo = "https://tr.mobiltv.net" + logo
        
        if kanal_adi and kanal_param:
            temp_kanal = {
                "name": kanal_adi,
                "img": logo,
                "param": kanal_param
            ...
            print(f"Kanal eklendi: {kanal_adi} - {kanal_param}")
...
def get_stream_urls(param, yayin=1):
    ...
    try:
        ...
        if decoded_streams and decoded_streams[0] and "mobiltv.net" in decoded_streams[0]:
            print(f"Stream URL bulundu: {decoded_streams[0][:100]}...")
            return decoded_streams
        else:
            print(f"Stream bulunamadı, yayın {yayin+1} deneniyor...")
...
if __name__ == "__main__":
    ...
    for i, kanal in enumerate(kanallar, 1):
        print(f"\n[{i}/{len(kanallar)}] Kanal işleniyor: {kanal['name']}")
        stream_urls = get_stream_urls(kanal['param'])
        
        if stream_urls and stream_urls[0]:
            # Dosya adı oluştur
            channel_slug = kanal['param'].replace("-", "_").replace("/", "_")
            ...
            if result == "success":
                ...
                print(f"✓ {kanal['name']} başarıyla eklendi")
            else:
                print(f"✗ {kanal['name']} dosya oluşturulamadı")
        else:
            print(f"✗ {kanal['name']} stream bulunamadı")

    playlist_file.close()
    print(f"\nPlaylist oluşturma tamamlandı!")
    print(f"Başarılı kanal sayısı: {successful_channels}/{len(kanallar)}")
